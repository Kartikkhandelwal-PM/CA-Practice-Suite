...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (session: any) => {
    ...
    if (persistedData) {
      ...
      if (data.complianceCategories) setComplianceCategories(data.complianceCategories);
    } else {
      if (user.id === 'demo-user-id') {
        setTasks(INIT_TASKS);
        setClients(INIT_CLIENTS);
        const demoUsers = [...INIT_USERS];
        ...
        const welcomeTask: Task = {
          id: 'KDK-1',
          title: 'Welcome to KDK Practice Suite!',
          clientId: '',
          type: 'Task',
          status: 'To Do',
          priority: 'Medium',
          ...
          recurring: 'One-time',
          description: 'This is your first task. Explore the dashboard, add clients, and manage your practice efficiently.',
          tags: ['welcome'],
          subtasks: [],
          comments: [],
          attachments: [],
          activity: [{ text: 'Account created', at: new Date().toISOString() }]
        };
