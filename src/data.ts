...
export const INIT_TASKS: Task[] = [
  { id: 'KDK-1', title: 'GSTR-3B Filing — October 2024', clientId: 'c1', type: 'GST', status: 'In Progress', priority: 'High', assigneeId: 'u2', reviewerId: 'u1', dueDate: fmt(subDays(today, 2)), createdAt: fmt(subDays(today, 19)), recurring: 'Monthly', description: 'File GSTR-3B for Agarwal Exports for October 2024. ITC claim pending verification.', tags: ['urgent', 'filing'], comments: [{ id: 'cm1', userId: 'u2', text: 'Bank statement received. Starting GSTR-2B reconciliation.', createdAt: fmt(today) }, { id: 'cm2', userId: 'u1', text: 'This is overdue! Penalty accruing. File today.', createdAt: fmt(subDays(today, 1)) }], attachments: [{ id: 'a1', name: 'Oct_Invoices.pdf', size: '2.1 MB', type: 'pdf' }, { id: 'a2', name: 'Bank_Statement.xlsx', size: '340 KB', type: 'xlsx' }], activity: [{ text: 'Moved to In Progress by Priya', at: fmt(today) }, { text: 'Auto-escalated (overdue)', at: fmt(today) }, { text: 'Created by Rajesh', at: fmt(subDays(today, 19)) }] },
  { id: 'KDK-2', parentId: 'KDK-1', issueType: 'Subtask', title: 'Collect purchase invoices', clientId: 'c1', type: 'GST', status: 'Completed', priority: 'Medium', assigneeId: 'u2', reviewerId: 'u1', dueDate: fmt(subDays(today, 5)), createdAt: fmt(subDays(today, 19)), recurring: 'One-time', description: '', tags: [], comments: [], attachments: [], activity: [] },
  { id: 'KDK-3', parentId: 'KDK-1', issueType: 'Subtask', title: 'Obtain bank statement', clientId: 'c1', type: 'GST', status: 'Completed', priority: 'Medium', assigneeId: 'u2', reviewerId: 'u1', dueDate: fmt(subDays(today, 4)), createdAt: fmt(subDays(today, 19)), recurring: 'One-time', description: '', tags: [], comments: [], attachments: [], activity: [] },
  { id: 'KDK-4', parentId: 'KDK-1', issueType: 'Subtask', title: 'Reconcile GSTR-2B vs books', clientId: 'c1', type: 'GST', status: 'In Progress', priority: 'High', assigneeId: 'u2', reviewerId: 'u1', dueDate: fmt(today), createdAt: fmt(subDays(today, 19)), recurring: 'One-time', description: '', tags: [], comments: [], attachments: [], activity: [] },
  { id: 'KDK-5', title: 'TDS Return Q2 — 26Q Filing', clientId: 'c2', type: 'TDS', status: 'In Progress', priority: 'High', assigneeId: 'u3', reviewerId: 'u1', dueDate: fmt(addDays(today, 8)), createdAt: fmt(subDays(today, 10)), recurring: 'Quarterly', description: 'Prepare and file Form 26Q for Q2 for Mehta Industries.', tags: ['tds', 'quarterly'], comments: [{ id: 'cm1', userId: 'u3', text: 'Salary data received. Preparing return now.', createdAt: fmt(subDays(today, 1)) }], attachments: [], activity: [{ text: 'Amit started work', at: fmt(subDays(today, 2)) }] },
  { id: 'KDK-6', title: 'Balance Sheet Preparation FY 2023-24', clientId: 'c3', type: 'Audit', status: 'Under Review', priority: 'Medium', assigneeId: 'u4', reviewerId: 'u2', dueDate: fmt(addDays(today, 25)), createdAt: fmt(subDays(today, 30)), recurring: 'Annual', description: 'Prepare balance sheet and P&L for Sharma & Sons.', tags: ['audit', 'annual'], comments: [], attachments: [{ id: 'a1', name: 'Trial_Balance.xlsx', size: '180 KB', type: 'xlsx' }], activity: [{ text: 'Sneha submitted for review', at: fmt(subDays(today, 1)) }] },
  { id: 'KDK-7', title: 'ROC Annual Return MGT-7', clientId: 'c4', type: 'ROC', status: 'Completed', priority: 'Low', assigneeId: 'u2', reviewerId: 'u1', dueDate: fmt(subDays(today, 5)), createdAt: fmt(subDays(today, 25)), recurring: 'Annual', description: 'File MGT-7 annual return for Patel Constructions on MCA portal.', tags: ['roc', 'completed'], comments: [{ id: 'cm1', userId: 'u2', text: 'Filed successfully. Challan receipt saved.', createdAt: fmt(subDays(today, 5)) }], attachments: [{ id: 'a1', name: 'MGT7_Filed.pdf', size: '520 KB', type: 'pdf' }], activity: [{ text: 'Task completed', at: fmt(subDays(today, 5)) }] },
  { id: 'KDK-8', title: 'ITR Filing — AY 2024-25', clientId: 'c5', type: 'ITR', status: 'To Do', priority: 'Medium', assigneeId: 'u3', reviewerId: 'u1', dueDate: fmt(addDays(today, 41)), createdAt: fmt(subDays(today, 5)), recurring: 'Annual', description: 'Prepare and file Income Tax Return for Verma Pharmaceuticals.', tags: ['itr', 'annual'], comments: [], attachments: [], activity: [{ text: 'Task created from template', at: fmt(subDays(today, 5)) }] },
  { id: 'KDK-9', title: 'GSTR-1 November — Outward Supplies', clientId: 'c6', type: 'GST', status: 'To Do', priority: 'High', assigneeId: 'u4', reviewerId: 'u2', dueDate: fmt(addDays(today, 21)), createdAt: fmt(subDays(today, 3)), recurring: 'Monthly', description: 'File GSTR-1 for Sunita Textiles LLP for November 2024.', tags: ['gst', 'monthly'], comments: [], attachments: [], activity: [{ text: 'Task auto-created (recurring)', at: fmt(subDays(today, 3)) }] },
  { id: 'KDK-10', title: 'Advance Tax Q3 Computation', clientId: 'c2', type: 'Advance Tax', status: 'To Do', priority: 'Medium', assigneeId: 'u5', reviewerId: 'u3', dueDate: fmt(addDays(today, 25)), createdAt: fmt(subDays(today, 2)), recurring: 'Quarterly', description: 'Compute Q3 advance tax liability for Mehta Industries.', tags: ['advance-tax'], comments: [], attachments: [], activity: [{ text: 'Task created', at: fmt(subDays(today, 2)) }] },
  { id: 'KDK-11', title: 'GST Reconciliation — GSTR-2B vs Books', clientId: 'c1', type: 'GST', status: 'Awaiting Info', priority: 'Medium', assigneeId: 'u4', reviewerId: 'u2', dueDate: fmt(addDays(today, 15)), createdAt: fmt(subDays(today, 8)), recurring: 'Monthly', description: 'Reconcile GSTR-2B auto-drafted ITC with purchase books for Agarwal Exports.', tags: ['reconciliation'], comments: [{ id: 'cm1', userId: 'u4', text: 'Pending 5 invoices from client. Sent reminder.', createdAt: fmt(subDays(today, 1)) }], attachments: [], activity: [{ text: 'Status changed to Awaiting Info', at: fmt(subDays(today, 1)) }] },
  { id: 'KDK-12', title: 'Statutory Audit FY 2023-24', clientId: 'c8', type: 'Audit', status: 'In Progress', priority: 'High', assigneeId: 'u3', reviewerId: 'u1', dueDate: fmt(addDays(today, 35)), createdAt: fmt(subDays(today, 45)), recurring: 'Annual', description: 'Full statutory audit for Raghunath Steel Works FY 2023-24.', tags: ['audit', 'statutory'], comments: [], attachments: [{ id: 'a1', name: 'Audit_Plan.pdf', size: '890 KB', type: 'pdf' }], activity: [{ text: 'Audit commenced', at: fmt(subDays(today, 10)) }] },
];
...
export const INIT_EMAILS: Email[] = [
  { id: 'e1', from: 'Ramesh Agarwal', fromEmail: 'ramesh@agarwalexports.in', clientId: 'c1', subject: 'Documents for GSTR-3B — October', preview: 'Please find attached purchase invoices and bank statements...', body: 'Dear Rajesh ji,\n\nPlease find attached documents for GSTR-3B filing for October 2024:\n\n1. Purchase invoices (47 total)\n2. Bank statements — HDFC Current Account\n3. Credit notes issued during the month\n\nKindly process by month-end.\n\nRegards,\nRamesh Agarwal', date: fmt(today), time: '10:32 AM', read: false, taskLinked: 'KDK-1', attachments: ['Oct_Invoices.pdf', 'Bank_Statement_Oct.xlsx'] },
  { id: 'e2', from: 'Mehta Industries HR', fromEmail: 'hr@mehtaind.in', clientId: 'c2', subject: 'Salary data for TDS calculation — Q2', preview: 'Hi, attaching salary register for Q2 TDS calculation...', body: 'Hi Amit,\n\nAttaching salary register and Form 12BA for Q2.\n\nTotal employees: 48\nTotal salary paid: ₹38,42,000\nTDS deducted: ₹3,12,400\n\nThanks,\nNeha Mehta', date: fmt(today), time: '9:15 AM', read: false, taskLinked: 'KDK-5', attachments: ['Salary_Register_Q2.xlsx'] },
  { id: 'e3', from: 'Income Tax Department', fromEmail: 'no-reply@incometax.gov.in', clientId: 'c3', subject: 'Notice u/s 139(9) — Defective Return', preview: 'Your ITR filed for AY 2024-25 has been classified as defective...', body: 'Dear Taxpayer,\n\nYour Income Tax Return for AY 2024-25 has been classified as Defective under Section 139(9).\n\nDefect: Tax computation mismatch in Schedule BP.\n\nPlease rectify within 15 days.', date: fmt(subDays(today, 1)), time: '8:00 AM', read: false, taskLinked: null, attachments: [] },
  { id: 'e4', from: 'Suresh Sharma', fromEmail: 'ceo@sharmasons.in', clientId: 'c3', subject: 'Request for meeting — Audit discussion', preview: 'I would like to schedule a meeting to discuss the audit findings...', body: 'Dear Rajesh ji,\n\nI would like to schedule a meeting to discuss the audit findings for FY 2023-24.\n\nAre you available Thursday or Friday afternoon?\n\nWarm regards,\nSuresh Sharma', date: fmt(subDays(today, 1)), time: '3:45 PM', read: true, taskLinked: null, attachments: [] },
  { id: 'e5', from: 'GST Portal', fromEmail: 'no-reply@gst.gov.in', clientId: 'c6', subject: 'GSTR-1 Filing Confirmation — October', preview: 'Your GSTR-1 for October 2024 has been successfully filed.', body: 'Dear Taxpayer,\n\nGSTR-1 for October 2024 has been successfully filed.\n\nGSTIN: 29AAQFS3388L1ZD\nFiling Date: ' + fmt(subDays(today, 3)) + '\nARN: AA2910240001234', date: fmt(subDays(today, 3)), time: '9:00 AM', read: true, taskLinked: null, attachments: ['GSTR1_Confirmation.pdf'] },
];
